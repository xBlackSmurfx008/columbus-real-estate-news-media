...
- **06:33 America/New_York** (cloud-scheduled routine — see "Automation" below):
  ...
  8. Keep market data fresh (accuracy pass): refresh the metro snapshot/ticker as always, AND rotate through the 14 rows of the `neighborhoods` table refreshing ~3 per run so all 14 stay current within a week. The table's price column is displayed as **"Typical Value"** and holds **Zillow's Home Value Index (ZHVI)** — the smoothed "typical home value" from each area's Zillow home-values page (e.g. `zillow.com/home-values/4426/dublin-oh/`). Use ZHVI, NOT single-month median sale price — median sale swings wildly in small neighborhoods (a few sales moved one area 25-60% month to month). Cross-check the YoY against Redfin/Rocket when possible. Apply via `node frontend/scripts/update-site-data.mjs <file.json>` using the `neighborhoods` key (writes to the `median` field); set any field you cannot verify to `"UNVERIFIED"` (the script leaves those unchanged) and NEVER guess a figure.
