...
- **06:33 America/New_York** (cloud-scheduled routine — see "Automation" below):
  ...
  7. Run `node frontend/scripts/publish-article.mjs <file.json>`. This command publishes the article `status='live'` after the deterministic editorial checks pass. The deterministic gate is the pre-publish quality bar; review and fixes happen post-publish. Never bypass the script or write a row directly.
  8. Hero requirement: every live article should receive a durable, reachable, story-specific hero with a unique content fingerprint as soon as possible after publish. Local `/images/heroes/` paths are not publication images; the branded editorial placeholder card is acceptable only as a temporary stand-in until the image workflow attaches the real hero. Images never block publication.
  ...
  10. Write a brief to `briefs/<date>.md` summarizing sources used, published article links, and which neighborhood rows you refreshed, then commit and push.
