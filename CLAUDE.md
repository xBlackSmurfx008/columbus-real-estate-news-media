...
- **06:33 America/New_York** (cloud-scheduled routine — see "Automation" below):
  ...
  8. Hero requirement (owner, 2026-08-14): NO live article may be imageless, ever. `publish-article.mjs` enforces this — if no image is supplied it generates a branded editorial-card placeholder (`scripts/editorial-card-lib.mjs`) into `frontend/public/images/heroes/` and sets `image_url` immediately. Commit that directory so the card deploys. Do not call Higgsfield from the cloud routine. The separate idempotent local Codex subscription job creates a real editorial illustration for each article whose `image_url` is NULL or still a `/images/heroes/` placeholder, normalizes it to 1600×900 WebP, and replaces the placeholder (a hero attach is the only live-article field it may touch). Every run must verify heroes with `npm run newsroom:audit-public-images` (checks every live article's image actually resolves over HTTP); on any missing or broken hero, re-run it with `--fix` in the same run to attach placeholders immediately, commit `frontend/public/images/heroes/`, and flag it in the brief.
  ...
  10. Write a brief to `briefs/<date>.md` summarizing sources used, links to the two new articles, and which neighborhood rows you refreshed, then commit and push (include `frontend/public/images/heroes/` so any placeholder heroes generated at publish deploy with the site).
