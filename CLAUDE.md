...
- **06:33 America/New_York** (cloud-scheduled routine — see "Automation" below):
  ...
  3. Run `cd frontend && DATABASE_URL=... npm run newsroom:coverage-calendar`. It prints what is due today, what has slipped, what lands in the next 14 days, and what we have missed, ranked, with one suggested pick per slot. Paste the block into today's brief and start from it — publishing two days ahead of a dated event is the only traffic amplifier we have measured. A calendar entry is a peg, not a story: verify it against a live source, and if the date has moved, `coverage-calendar.mjs cancel` it and publish nothing. Follow `frontend/docs/COVERAGE_CALENDAR.md`; every entry cites the article or brief its date came from, and new dates found today should be added to `frontend/content/coverage-calendar/seed.json` the same way.
  4. Research via web search + the source list below. Pull local, government, and national coverage.
  5. Pick at most 1 strong real estate story and 1 strong lifestyle story not already covered. A Neighborhoods story consumes the real-estate slot. Publish zero rather than force a weak or duplicative daily item.
  6. Verify facts across sources; if a story originates on another outlet, write original local analysis and link back to it (see `.claude/skills/cren-copywriting`).
  7. Draft each worthwhile article and its source, claim, entity, SEO, tag, and image metadata per `frontend/prompts/ARTICLE_WRITING.md` and `frontend/docs/article-submission.schema.json`. Body format is Markdown only. Return `NEEDS_REPORTING` instead of filling evidence gaps.
  8. Run `node frontend/scripts/publish-article.mjs <file.json>`. This command publishes the article `status='live'` after the deterministic editorial checks pass. The deterministic gate is the pre-publish quality bar; review and fixes happen post-publish. Never bypass the script or write a row directly.
  9. Hero requirement: every live article should receive a durable, reachable, story-specific hero with a unique content fingerprint as soon as possible after publish. Local `/images/heroes/` paths are not publication images; the branded editorial placeholder card is acceptable only as a temporary stand-in until the image workflow attaches the real hero. Images never block publication.
  10. Keep market data fresh (accuracy pass): refresh the ticker as always, then run `DATABASE_URL=... node frontend/scripts/refresh-market-data.mjs` — one command that pulls FREE public feeds (Zillow Research ZHVI for the neighborhood "Typical Value" column + YoY, Zillow ZORI for rents, FRED for the 30-yr mortgage rate) and updates the DB automatically. No API keys, no scraping. It only writes what it can resolve and leaves the rest unchanged, so it never fabricates. (Short North, Clintonville, and Franklinton aren't in Zillow's neighborhood file — leave their prior values or update from a named source if you find one; never guess.) The metro Median Sale Price / Active Listings / Days on Market in the snapshot come from the Columbus REALTORS monthly report — update those from that report via `update-site-data.mjs` (`market_snapshot` key) when a new month is out. Both refresh scripts and `update-site-data.mjs` now re-export `frontend/content/snapshot/public-data.json` automatically, so the DB-outage fallback can never drift behind the database — commit that file with the refresh. Finish the pass with `DATABASE_URL=... npm run verify:market-consistency` (in `frontend/`); it fails if the database, the committed fallback, or `hero_stats` disagree about any metric. All public market numbers come from one canonical module, `frontend/lib/market-data.ts` — never read `market_snapshot`, `market_observations`, or `hero_stats` directly in a page or component.
  11. Health check (this routine is the monitoring, because nothing else runs). From `frontend/`: `npm run newsroom:uptime`, then `DATABASE_URL=... npm run verify:site -- --target production`. Record the result in the brief every day, including when everything passes — a silent pass is what a broken monitor looks like. Escalate in the brief and by push notification if the homepage or `/blog` renders zero articles, if `verify:site` reports a new blocking failure, or if either command cannot run at all. Background: the `uptime.yml` GitHub Action has never worked. It fails in about two seconds with no runner assigned (`runner_id: 0`, no logs), which is an account-level GitHub Actions block, not a workflow bug, so 116 scheduled runs "failed" without ever executing a check or alerting anyone. Until that billing issue is resolved this step is the only live monitoring the property has.
  12. Write a brief to `briefs/<date>.md` summarizing sources used, published article links, which neighborhood rows you refreshed, and the step 11 health result, then commit and push.
- Publication policy (owner update, 2026-08-25, given live in-session and superseding the 2026-08-21 staging policy): publish live by default. The deterministic editorial gate in `publish-article.mjs` is the publication bar — an article that passes it goes live immediately, with no pre-publish human approval. Review and corrections happen post-publish via the admin panel. The owner prefers Telegram (not email) for notifications about publishes, errors, and reports.

## Automation
The daily text run above executes as a cloud-scheduled Claude Code routine (not a local cron job — see `claude.ai/code/routines`). It authenticates to NeonDB via a `DATABASE_URL` embedded in the routine's own configuration (not committed to this repo). The repository gate is the safety boundary: even if routine instructions drift, `publish-article.mjs` only publishes articles that pass the full deterministic editorial gate. A Vercel Workflow will replace the local image LaunchAgent after cloud image generation and production verification succeed.

## Channels
Daily runs publish gate-passing articles live in the `articles` table (owner policy, 2026-08-25). Public social copy still goes out only after authenticated human approval.
