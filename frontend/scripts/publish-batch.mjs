...
for (let i = 0; i < articles.length; i++) {
  ...
  const [row] = await sql`
    INSERT INTO articles (id, canonical_slug, status, featured, category, category_class, icon, title, excerpt, body, author, date, read_time, area_slug, topic_slug, tags, image_url, meta_description, image_alt, image_caption, fact_checked_at)
    VALUES (${id}, ${slug}, 'draft', false, ${a.category}, ${a.category_class ?? "card-img-market"}, ${a.icon ?? "$"},
