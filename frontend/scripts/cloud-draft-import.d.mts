...
export type CloudDraftBatch = { date: string; commit: string; artifacts: CloudDraftArtifact[]; runReceipt?: {
  storyResult: 'ARTIFACTS_COMMITTED' | 'NO_QUALIFYING_STORY'; completedAt: string; path: string; blobSha: string;
} | null };
...
export function runCloudImport(sql: EditorialSql,source:()=>Promise<CloudDraftBatch>,options?:{apply?:boolean;now?:Date}):Promise<{ok:boolean;status:string;commit:string;date:string;results:Record<string,unknown>[];runReceipt:{present:boolean;quietRunRecorded:boolean};published:number}>;
